...
def main():
    ...
    from src.hid_defender.config import (
        IS_WINDOWS, IS_MACOS, IS_LINUX, LOG_PATH, 
        KEYSTROKE_THRESHOLD, MALICIOUS_PATTERNS, RECENT_SEEN
    )

    # Import modules
    from src.hid_defender.logging_setup import init_logger, log_event
    from src.hid_defender.keystroke_monitor import KeystrokeMonitor, PYNPUT_AVAILABLE
    from src.hid_defender.device_monitor import get_macos_usb_devices
    from src.hid_defender.device_validator import (
        get_whitelist, run_baseline_setup, parse_device, 
        evaluate, should_debounce
    )
    from src.hid_defender.alert_system import play_alert_sound, show_alert, lock_workstation
